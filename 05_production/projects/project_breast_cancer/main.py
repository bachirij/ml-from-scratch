...
# /predict/decision_tree endpoint to make predictions
    ...
    try:
        ...
        features = np.array([
            [request.mean_radius, 
             ...
             request.worst_fractal_dimension]]) # Size (1, 30): 1 sample, 30 features
        scaled_features = scaler.transform(features) # Scale features with the loaded scaler
        prediction = models.get("decision_tree").predict(scaled_features) # Size (1,): 1 prediction for 1
        prediction_proba = models.get("decision_tree").predict_proba(scaled_features) # Get the probability of obtaining the class 1
        return {"prediction": int(prediction.flatten()[0]), # return the single prediction as an integer
                "prediction_probability": float(prediction_proba[0])} 
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

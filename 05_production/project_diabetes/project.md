...
> Path: `05_production/project_diabetes/project.md`
...
using FastAPI, and how to containerize it with Docker. Rather than running predictions
inside a notebook, the model is exposed via HTTP endpoints — meaning any client (a
browser, a script, another application) can send features and receive a prediction in
return.

This is the first step toward production-ready ML: moving from a notebook experiment to a
deployed, callable service that runs identically on any machine.
...
```
05_production/project_diabetes/
...
COPY main.py schemas.py linear_regression.py linear_regression_scratch.pkl /app/
...
- `-t diabetes-api` → names the image `diabetes-api`
...
- `diabetes-api` → the name of the image to run
...
```bash
docker stop $(docker ps -q --filter ancestor=diabetes-api)
```
...
Click on `POST /predict`, then `Try it out`, paste the following JSON body, and click
`Execute`:
